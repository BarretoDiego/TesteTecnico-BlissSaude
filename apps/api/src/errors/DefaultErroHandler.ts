...
import { logger } from "@/utils/SBLogger";
...
export function DefaultErroHandler(
	...
	if (error instanceof ZodError) {
		...
		return sbFail(res, req, definition.httpStatus, {
	...
	if (SBError.isSBError(error)) {
		...
		return sbFail(res, req, error.httpStatus, error.toDetail());
	}
