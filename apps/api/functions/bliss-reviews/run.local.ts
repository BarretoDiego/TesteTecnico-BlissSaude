/**
 * Servidor local isolado do microserviço `bliss-reviews`.
 *
 * Sobe **apenas este domínio**, na porta 4002 (sobrescreva com `SERVICE_PORT`) — que é como ele roda em
...
async function main(): Promise<void> {
	...
	await runLocal({
		...
		buildApp,
		onShutdown: closeDb,
		port: Number(process.env.SERVICE_PORT ?? 4002),
	});
}
